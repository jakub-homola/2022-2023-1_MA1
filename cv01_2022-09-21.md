...
komunikace - nejlíp MA Teams nebo mail
...
příští týden 28.9. státní svátek. není čas ztrácet čas. takže udělám video které vám pošlu
...
ze střední znáte $N, Z, Q, R$, ale tady se to značí $\N$, $\Z$, $\mathbb{Q}$, $\R$
...
- závorky se můžou ale nemusí dávat
    - $\forall x \in \R \enspace \exists y \in \R : x+y=7$
    - $(\forall x \in \R) \enspace (\exists y \in \R) : x+y=7$
...
příklad: mejme množiny $A=\{2,4,5\}$, $B=\{1,2,3\}$. urči
- $A \cup B$
- $A \cap B$
- $A \setminus B$
