...
funkce se dá zadat několika způsoby. Každá funkce ale má svůj definiční obor. Definiční obor je **množina** reálných čísel.

funkce se dá zadat např. definičním oborem a předpisem, tj. $f(x) = x^2 + \ln(x) - \frac{14}{x+2}$, $Df=(3,7)$.
...
- např. $f(x) = \frac{1}{x}$ nedává smysl pro $x=0$
