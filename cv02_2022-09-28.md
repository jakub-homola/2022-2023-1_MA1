...
Prvnímu předpokladu budeme říkat základní případ. Druhému předpokladu se říká indukční krok.

Jak zjistíme, že spadnou všechny kostky domina? Nejdřív dokážeme, že spadne první kostka. A potom dokážeme, že všechny kostky, které spadnou, shodí i tu další.

Jak zjistíme, že jsou v množině $M$ *všechna* přirozená čísla? Nejdřív dokážeme, že je v množině $M$ první číslo (jednička). A potom dokážeme, že pro každé číslo, které v množině $M$ je, je v $M$ i to další.
...
Chceme zjistit, jestli výroková forma $1+2+3+\dots+n = \frac{n\cdot(n+1)}{2}$ platí pro všechna přirozená čísla. Označím si množinu $M$ jako množinu všech přirozených čísel, pro která forma platí. $M = \lbrace k \in \mathbb{N} : 1+2+3+\dots+k = \frac{k\cdot(k+1)}{2} \rbrace$. Teď chceme dokázat, že $M = \mathbb{N}$. K tomu nám stačí dokázat předpoklady věty o principu matematické indukce. Začneme základním případem. $1 \in M$, protože $1 = \frac{1\cdot(1+1)}{2}=1$. Následuje indukční krok. Předpokládejme, že $n \in M$, tedy že $1+2+3+\dots+n = \frac{n\cdot(n+1)}{2}$. Máme ukázat, že pak $(n+1) \in M$, tj. máme ukázat že $1+2+3+\dots+n+(n+1) = \frac{(n+1)\cdot((n+1)+1)}{2}$. Protože $1+2+3+\dots+n+(n+1) = \frac{n\cdot(n+1)}{2} + (n+1) = \frac{n\cdot(n+1) + 2\cdot(n+1)}{2} = \frac{(n+2)(n+1)}{2} = \frac{(n+1)((n+1)+1)}{2}$, tak $(n+1) \in M$. Jak jsme ukázali, $1 \in M$ a zároveň $\forall n \in M : (n+1) \in M$, tedy $M = \mathbb{N}$. $\blacksquare$
