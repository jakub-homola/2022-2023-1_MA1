...
> $\quad \implies$  
> $\displaystyle \int_{a}^{b} u v^\prime \ \mathrm{d}x = |\dots per\ pertes\dots| = \left[ uv \right]_a^b - \int_{a}^{b} u^\prime v \ \mathrm{d}x$
...
Další příklady:
- $\displaystyle \int_{0}^{1} x^3 e^{x^2} \ \mathrm{d}x$
